#!/usr/bin/env python3
"""
Framefetch_FW13_1.py Framework physical hardware dashboard for the Framework 13 ASCII dashboard.
...
from config import (
    ...
    COLOR_CPU_TEMP_OK, COLOR_CPU_TEMP_WARNING, COLOR_CPU_TEMP_CRITICAL,
    GPU_WARNING_PERCENT, GPU_CRITICAL_PERCENT,
    COLOR_GPU_OK, COLOR_GPU_WARNING, COLOR_GPU_CRITICAL,
    RAM_WARNING_PERCENT, RAM_CRITICAL_PERCENT,
    COLOR_RAM_OK, COLOR_RAM_WARNING, COLOR_RAM_CRITICAL,
    COLOR_RAM_MODULE_SIZE, COLOR_RAM_MODULE_SPEED,
    SSD_WARNING_PERCENT, SSD_CRITICAL_PERCENT,
    COLOR_SSD_OK, COLOR_SSD_WARNING, COLOR_SSD_CRITICAL,
    BATTERY_CRITICAL_PERCENT, BATTERY_WARNING_PERCENT,
    COLOR_BATTERY_OK, COLOR_BATTERY_WARNING, COLOR_BATTERY_CRITICAL,
    FAN_MAX_RPM_FALLBACK, FAN_WARNING_PERCENT, FAN_CRITICAL_PERCENT,
    COLOR_FAN_OK, COLOR_FAN_WARNING, COLOR_FAN_CRITICAL,
    COLOR_WIFI, COLOR_BLUETOOTH, COLOR_ETHERNET,
    COLOR_DISPLAYPORT, COLOR_POWER_DELIVERY,
    LOGO_COLOR, WATCH_LOGO_COLOR_MODE, WATCH_LOGO_COLORS,
    PORT_OVERRIDES, PHYSICAL_PORT_TYPE, FRAMEWORK_SODIMM_DMI_ORDER,
)
...
def usage_color(
    ...
    """Return a configurable traffic-light color for a percentage."""
...
def cpu_usage_color(percent: float) -> str:
    return usage_color(percent, CPU_WARNING_PERCENT, CPU_CRITICAL_PERCENT,
                       COLOR_CPU_OK, COLOR_CPU_WARNING, COLOR_CPU_CRITICAL)


def gpu_usage_color(percent: float) -> str:
    return usage_color(percent, GPU_WARNING_PERCENT, GPU_CRITICAL_PERCENT,
                       COLOR_GPU_OK, COLOR_GPU_WARNING, COLOR_GPU_CRITICAL)


def ram_usage_color(percent: float) -> str:
    return usage_color(percent, RAM_WARNING_PERCENT, RAM_CRITICAL_PERCENT,
                       COLOR_RAM_OK, COLOR_RAM_WARNING, COLOR_RAM_CRITICAL)


def ssd_usage_color(percent: float) -> str:
    return usage_color(percent, SSD_WARNING_PERCENT, SSD_CRITICAL_PERCENT,
                       COLOR_SSD_OK, COLOR_SSD_WARNING, COLOR_SSD_CRITICAL)


def battery_color(percent: float) -> str:
    """Return a battery color, where lower percentages are worse."""
    ...
    return COLOR_BATTERY_OK


def temp_color(temp: float) -> str:
    """Return the configured CPU temperature color."""
    if temp >= CPU_TEMP_CRITICAL:
        return COLOR_CPU_TEMP_CRITICAL
    if temp >= CPU_TEMP_WARNING:
        return COLOR_CPU_TEMP_WARNING
    return COLOR_CPU_TEMP_OK


def fan_color(percent: float) -> str:
    return usage_color(percent, FAN_WARNING_PERCENT, FAN_CRITICAL_PERCENT,
                       COLOR_FAN_OK, COLOR_FAN_WARNING, COLOR_FAN_CRITICAL)
...
def cache_hardware() -> tuple[bool, str]:
    ...
    if not out:
        return False, "dmidecode is not readable. Run this command with sudo."

    parsed = _parse_dmidecode_memory(out)
    if not parsed:
        return False, "No RAM slots could be read from SMBIOS."
    ...
    try:
        ...
        return False, f"Unable to write cache: {e}"
...
def render_port_lines(
    ...
    for x in content:
        ...
        if plain.startswith("DP "):
            line = line.replace("DP", ansi("DP", COLOR_DISPLAYPORT), 1)

        elif plain.startswith("ETHERNET "):
            line = line.replace("ETHERNET", ansi("ETHERNET", COLOR_ETHERNET), 1)

        elif plain.startswith("ETH "):
            line = line.replace("ETH", ansi("ETH", COLOR_ETHERNET), 1)

        elif plain.startswith("PD charging"):
            line = line.replace(
                "PD charging",
                ansi("PD charging", COLOR_POWER_DELIVERY),
                1
            )
            ...
            line = line.replace(
                "PD out",
                ansi("PD out", COLOR_POWER_DELIVERY),
                1
            )
        ...
        if m:
            try:
                pct = float(m.group(2))
                line = ansi(line, ssd_usage_color(pct))
            except ValueError:
                pass
...
def calibrate_typec_interactive(port_number: int) -> tuple[bool, str]:
    ...
    if port_number not in (1, 2, 3, 4):
        return False, "Port must be 1, 2, 3, or 4."

    print(
        f"\nPhysical Type-C calibration PORT {port_number}\n"
        "1) Leave that specific port EMPTY.\n"
        "2) The other ports can remain connected as they are.\n"
    )
    input("Press Enter when the PORT is empty... ")
    before = _typec_state_snapshot()

    print(
        "\nNow connect a USB-C device to that PORT.\n"
        "A monitor, charger, phone, hub, or another Type-C device is fine."
    )
    ...
    if len(candidates) != 1:
        detail = ", ".join(f"port{x}" for x in candidates) or "none"
        return False, (
            "Unable to identify exactly one changed Type-C connector. "
            f"Candidates: {detail}. Try again without connecting or disconnecting anything else "
            "during calibration."
        )
...
def calibrate_usb_generic(port_number: int) -> tuple[bool, str]:
    ...
    print(
        f"\nPhysical USB calibration PORT {port_number}\n"
        "--------------------------------------------------\n"
        "Disconnect the USB device you want to use as a probe from the PORT.\n"
        "You do NOT need to disconnect devices from the other ports.\n"
        "DP monitors and the charger can remain connected.\n"
    )
    input("Press Enter when the USB probe is DISCONNECTED... ")

    before = _usb_signature_snapshot()

    print(
        f"\nNow connect the USB probe to Framework PORT {port_number}.\n"
        "Wait until the device is detected/mounted."
    )
    input("Press Enter AFTER the device appears in the system... ")
    ...
    if len(appeared) == 0:
        return False, (
            "No new USB signature appeared.\n"
            "If you are calibrating a DisplayPort monitor or charger, this is expected: "
            f"usa `--calibrate-typec {port_number}`.\n"
            "If it is an SSD/phone/USB drive instead, make sure it is actually "
            "enumerated by the system and try again."
        )
    ...
    if len(appeared) > 1:
        ...
        return False, (
            "Multiple USB signatures appeared at the same time, so the program cannot "
            "choose one safely:\n  "
            + "\n  ".join(details)
            + "\n\nIf you connected a hub/dock, this can be normal. "
              "In that case the whole hub/group needs to be calibrated."
        )
...
def calibrate_port(port_number: int) -> tuple[bool, str]:
    ...
    if not candidates:
        return False, (
            "No suitable USB device detected. "
            "Connect an iPhone/iPad with tethering or a USB data device to port "
            f"{port_number} and try again."
        )

    # If several active USB-network devices exist, calibration would be ambiguous.
    if len(candidates) > 1:
        labels = ", ".join(x[0] for x in candidates)
        return False, f"Ambiguous calibration: multiple active USB devices detected: {labels}"

    label, devpath = candidates[0]
    sig = _usb_topology_signature(devpath)
    if not sig:
        return False, f"Unable to determine the USB topology for {label}."
...
def calibrate_dp_partner(port_number: int) -> tuple[bool, str]:
    ...
    if target_tc is None:
        return False, f"PORT {port_number} has no Type-C calibration. Run --calibrate-typec {port_number}."
    if not _typec_has_partner(target_tc):
        return False, f"No Type-C partner is connected to PORT {port_number}."
    external = [d for d in displays() if not d.builtin]
    if not external:
        return False, "DRM/KScreen does not detect any external display."
    fp = _typec_partner_fingerprint(target_tc)
    if not fp:
        return False, "The firmware does not expose enough partner metadata; run --diagnostics and inspect TYPE-C PARTNER DETAILS."
    data = _load_port_map()
...
def clear_dp_partner_calibration() -> tuple[bool, str]:
    ...
    _save_port_map(data)
    return True, "DisplayPort partner fingerprints removed."
...
def autodetect_ports(
    ...
    for iface, product, rx, tx, physical, _sig in _usb_networks(nmcli_out):
        ...
        result[physical].append(f"{ansi('ETH',COLOR_ETHERNET)} ↓{compact_rate(rx)} ↑{compact_rate(tx)}")
        occupied_by_known_service.add(physical)
    ...
    if _external_power_online():
        ...
        if len(sinks) == 1:
            ...
            result[physical].append(f"{ansi('PD charging',COLOR_POWER_DELIVERY)} {watts:.0f}W" if watts else ansi('PD charging',GOLD))
            occupied_by_known_service.add(physical)
    ...
    if unresolved:
        ...
        for d in unresolved:
            ...
            result[physical].append(f"{ansi('DP',COLOR_DISPLAYPORT)} {d.width}x{d.height}{hz}")
            occupied_by_known_service.add(physical)
...
def colored_percent(value: Optional[float], kind: str) -> str:
    ...
    if color_fn is None:
        return f"{value:.0f}%"
    return ansi(f"{value:.0f}%", color_fn(value))
...
def color_sodimm_value(line: str, kind: str) -> str:
    if kind == "size" and line and line != "EMPTY":
        return ansi(line, COLOR_RAM_MODULE_SIZE)

    if kind == "speed":
        m = re.match(r"^(.*?)(\d{3,5})(.*)$", line)
        if m:
            return f"{m.group(1)}{ansi(m.group(2), COLOR_RAM_MODULE_SPEED)}{m.group(3)}"
...
def render(s: Snapshot, logo_color: Optional[str] = None) -> str:
    ...
    ram_bar = ansi(ram_bar_plain, ram_usage_color(rpct))
    ram_pct = ansi(f"{rpct:.0f}%", ram_usage_color(rpct))

    temp = s.cpu_temp
    temp_txt = f"TEMP {colored_temp_value(temp)}"

    fan_txt = f"FAN: {colored_percent(s.fan_percent, 'fan')}" if s.fan_percent is not None else "FAN: N/A"

    cpu_freq = f"{s.cpu_freq:.1f}GHz" if s.cpu_freq is not None else ""
    gpu_freq = f"{s.gpu_freq:.1f}GHz" if s.gpu_freq is not None else ""
    cpu_pct = colored_percent(s.cpu_usage, "cpu")
    gpu_pct = colored_percent(s.gpu_usage, "gpu")
    ...
    if disk:
        dpct = disk.pct
        dbar = ansi(percent_bar(dpct, SSD_BAR_WIDTH), ssd_usage_color(dpct))
        dpct_txt = ansi(f"{dpct:.0f}%", ssd_usage_color(dpct))
    ...
    net_name = ansi(net_name_plain, COLOR_WIFI)
    ...
    lines.append(
        f"│{portline(1,2)}│"
        f"{left_gap}│"
        f"         │{ansi(shift_ascii_line(LOGO[0], LOGO_SHIFT_RIGHT, 35), logo_color)}│   "
        f"│{fit(dline2,16)}│    "
    ...
    lines.append(
        f"│{portline(1,3)}│"
        f"{left_gap}│"
        f"         │{ansi(shift_ascii_line(LOGO[1], LOGO_SHIFT_RIGHT, 35), logo_color)}│   "
        f"└────────────────┘    "
    ...
    lines.append(
        f"│{portline(1,4)}│"
        f"{left_gap}│"
        f"         │{ansi(shift_ascii_line(LOGO[2], LOGO_SHIFT_RIGHT, 35), logo_color)}│                         "
    ...
    lines.append(
        f"│{portline(1,5)}│"
        f"{left_gap}│"
        f"         │{ansi(shift_ascii_line(LOGO[3], LOGO_SHIFT_RIGHT, 35), logo_color)}│   "
        f"┌─────────────────┐   "
    ...
    lines.append(
        "└───────────────┘"
        f"{left_gap}│"
        f"         │{ansi(shift_ascii_line(LOGO[4], LOGO_SHIFT_RIGHT, 35), logo_color)}│   "
        f"│ {fit(cpu_name,15)} │   "
    ...
    lines.append(
        f"{left_empty}│"
        f"         │{ansi(shift_ascii_line(LOGO[5], LOGO_SHIFT_RIGHT, 35), logo_color)}│   "
        f"│ {fit('',15)} │   "
    ...
    lines.append(
        "┌────PORT─2─────┐"
        f"{left_gap}│"
        f"         │{ansi(shift_ascii_line(LOGO[6], LOGO_SHIFT_RIGHT, 35), logo_color)}│   "
    ...
    lines.append(
        f"│{portline(2,0)}│"
        f"{left_gap}│"
        f"         │{ansi(shift_ascii_line(LOGO[7], LOGO_SHIFT_RIGHT, 35), logo_color)}│   "
        f"│{fit('',17)}│   "
    ...
    lines.append(
        f"│{portline(2,1)}│"
        f"{left_gap}│"
        f"         │{ansi(shift_ascii_line(LOGO[8], LOGO_SHIFT_RIGHT, 35), logo_color)}│   "
        f"│ {fit(gpu_name,15)} │   "
        f"│{fit('',10)}│  │{fit('',10)}│   │ "
        f"│{portline(4,1)}│"
    )
    ...
    lines.append(
        f"│{portline(2,2)}│"
        f"{left_gap}│"
        f"         │{ansi(shift_ascii_line(LOGO[9], LOGO_SHIFT_RIGHT, 35), logo_color)}│   "
    ...
    lines.append(
        f"│{portline(2,3)}│"
        f"{left_gap}│"
        f"         │{ansi(shift_ascii_line(LOGO[10], LOGO_SHIFT_RIGHT, 35), logo_color)}│   "
        f"└─────────────────┘   "
        f"│{fit('',10)}│  │{fit('',10)}│   │ "
        f"│{portline(4,3)}│"
    )
    ...
    lines.append(
        f"│{portline(2,4)}│"
        f"{left_gap}│"
        f"         │{ansi(shift_ascii_line(LOGO[11], LOGO_SHIFT_RIGHT, 35), logo_color)}│                         "
        f"└──────────┘  └──────────┘   │ "
        f"│{portline(4,4)}│"
    )
    ...
    lines.append(
        f"{left_empty}│                                                        "
        f"│{fit('BATTERIES',38)}│    │"
        f"│{fit(ansi('BT:', COLOR_BLUETOOTH) + (' ' + bt[0] if bt[0] else ''),19)}│"
    )
...
def watch_cached(initial: Snapshot, interval: float = 3.0) -> None:
    ...
    try:
        while True:
            ...
            if WATCH_LOGO_COLOR_MODE.lower() == "cycle" and WATCH_LOGO_COLORS:
...
def main() -> int:
    ...
    if args.calibrate_display is not None:
        print(
            "The old display calibration is deprecated because it binds the monitor "
            "to a port instead of the physical connector.\n"
            f"Use: --calibrate-typec {args.calibrate_display}"
        )
        return 2
